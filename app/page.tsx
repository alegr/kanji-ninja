...
// ── mutable singleton state (avoids re-render churn) ──────────────────────
let S: AppState = {
...
function markK(k: string, s: string) {
  ...
  S.prog[k].status = s
  const wk = wkKey()
  if (!S.prog[k].weekSeen.includes(wk)) S.prog[k].weekSeen.push(wk)
}
...
export default function HomePage() {
  ...
  const saveState = useCallback(() => {
    ...
    saveTimer.current = setTimeout(() => {
      fetch('/api/progress', {
        ...
        if (el) { el.className='sync saved'; el.textContent='✓ saved' }
  ...
  if (status === 'loading' || status === 'unauthenticated') {
    return <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',color:'var(--text2)'}}>Loading…</div>
  }
  ...
  function go(id: string, btn?: HTMLElement | null) {
    document.querySelectorAll('.panel').forEach(p => p.classList.remove('on'))
    document.querySelectorAll('.nb').forEach(b => b.classList.remove('on'))
    document.getElementById('panel-'+id)?.classList.add('on')
    if (btn) btn.classList.add('on')
    const fns: Record<string, ()=>void> = { dash, fc: initFC, quiz: initQ, prog: renderProg, week: renderWeek }
    fns[id]?.()
  }

  function setLv(lv: string, btn: HTMLElement) {
    S.lv = lv
  ...
  function dash() {
    ...
    pbarsEl.innerHTML = ''
    LEVELS.forEach(lv => {
      const data = KANJI_DATA[lv] ?? []
      const l = data.filter((k:KanjiEntry) => ['learning','mastered'].includes(st(k.k))).length
      const m = data.filter((k:KanjiEntry) => st(k.k)==='mastered').length
      totalL += l; totalM += m
      const el = document.getElementById('s-'+lv)
      if (el) el.textContent = l+'/'+data.length
    ...
    const tk = todayK()
    const lblEl = document.getElementById('today-lbl')
    if (lblEl) lblEl.textContent = tk.length ? tk.length+' kanji ready' : 'All caught up! 🎉'
    const row = document.getElementById('today-row')
    if (!row) return
    row.innerHTML = ''
    tk.forEach(k => {
      const lv = lvOf(k.k), s = st(k.k)
      const d = document.createElement('div')
      d.className = 'mk '+(s==='mastered'?'mastered lv-'+lv:s==='learning'?'learning':s==='seen'?'seen':'')
      d.textContent = k.k; d.title = k.m
      d.onclick = () => go('fc', document.querySelectorAll<HTMLElement>('.nb')[1])
      row.appendChild(d)
    })
  }
  ...
  // ── flashcards ──────────────────────────────────────────────────────────
  function initFC() {
    const learning = shuf(allK(S.lv).filter(k => st(k.k)==='learning'))
    const unseen   = shuf(allK(S.lv).filter(k => st(k.k)==='unseen'))
    const seen_    = shuf(allK(S.lv).filter(k => st(k.k)==='seen'))
    const master   = shuf(allK(S.lv).filter(k => st(k.k)==='mastered'))
    S.q = [...learning,...unseen,...seen_,...master].slice(0,20)
    S.qi = 0; S.flipped = false
    renderFC()
  }
  ...
  function renderFC() {
    ...
    if (st(k.k)==='unseen') markK(k.k, 'seen')
    dash()
  }
  ...
  function flip() {
    ...
    const k = S.q[S.qi]
    const lv = lvOf(k.k)
    const vocabHtml = k.vocab.map(v =>
      `<div class="vi"><span class="vw">${v.w}</span><span class="vr">${v.r}</span><span class="vm">${v.m}</span></div>`
    ).join('')
    const back = document.getElementById('fc-back')
    if (back) back.innerHTML = `
      <div class="bkanji" style="color:${LV_COLOR[lv]}">${k.k}</div>
      <div class="bmean">${k.m}</div>
      <div class="rtags">
        <div class="rtag"><span class="rtag-l">音読み</span>${k.on}</div>
        <div class="rtag"><span class="rtag-l">訓読み</span>${k.kun}</div>
        <div class="rtag"><span class="rtag-l">画数</span>${k.strokes}画</div>
        <div class="rtag" style="border-color:${LV_COLOR[lv]};color:${LV_COLOR[lv]}">${lv.toUpperCase()}</div>
      </div>
      <div class="vlabel">Example Vocabulary</div>
      ${vocabHtml}
      <div class="sbox">✏️ ${k.stroke_tip}</div>`
  ...
  function rate(r: string) {
    ...
    const wk = wkKey()
    if (!S.prog[k.k].weekSeen.includes(wk)) S.prog[k.k].weekSeen.push(wk)
    ...
    if (S.qi >= S.q.length) {
      ...
      document.getElementById('rrow')?.classList.add('hidden')
      const fcK=document.getElementById('fc-k'); if(fcK) fcK.textContent='🎉'
      dash(); return
    }
  ...
  // ── quiz ─────────────────────────────────────────────────────────────────
  function setQT(t: string, btn: HTMLElement) {
    S.qt = t
  ...
  function upQ() {
    const sc=document.getElementById('q-sc');  if(sc)  sc.textContent  = String(S.qsc)
    const ok=document.getElementById('q-ok');  if(ok)  ok.textContent  = String(S.qok)
    const bd=document.getElementById('q-bd');  if(bd)  bd.textContent  = String(S.qbd)
  }
  function nextQ() {
    const pool = shuf(allK(S.lv))
    ...
    const nxt=document.getElementById('q-next');  if(nxt)   nxt.style.display='none'
    if (!el) return
    el.innerHTML = ''
    opts.forEach(o => {
      const b = document.createElement('div')
      b.className='qo'; b.textContent=o; b.onclick=()=>ansQ(o,cor,b); el.appendChild(b)
    })
  }
  function ansQ(ch: string, cor: string, btn: HTMLElement) {
    if (S.qa || !S.qk) return; S.qa = true
    const ok = ch === cor
    document.querySelectorAll('.qo').forEach(b => {
      b.classList.add('done')
      if (b.textContent===cor) b.classList.add('ok')
    ...
    const wk = wkKey()
    if (!S.prog[S.qk!.k]) S.prog[S.qk!.k]={status:st(S.qk!.k),ease:0,weekSeen:[]}
    if (!S.prog[S.qk!.k].weekSeen.includes(wk)) S.prog[S.qk!.k].weekSeen.push(wk)
    upQ(); bumpStreak(); saveState()
    const nxt=document.getElementById('q-next'); if(nxt) nxt.style.display='block'
  }

  // ── progress ─────────────────────────────────────────────────────────────
  function renderProg() {
    const el = document.getElementById('prog-sections')
    if (!el) return; el.innerHTML = ''
    LEVELS.forEach(lv => {
      const h = document.createElement('div'); h.className='psec'
      h.style.color = LV_COLOR[lv]; h.textContent = lv.toUpperCase()+' — '+(KANJI_DATA[lv]?.length??0)+' Kanji'
      el.appendChild(h)
      const g = document.createElement('div'); g.className='pg'
      KANJI_DATA[lv]?.forEach((k:KanjiEntry) => {
        const s = st(k.k)
        const d = document.createElement('div')
        d.className='pk '+s+(s==='mastered'?' lv-'+lv:'')
        d.textContent=k.k; d.title=k.k+' — '+k.m+' ['+s+']' 
        d.onclick=()=>toast(k.k+' · '+k.m+' · '+k.on)
        g.appendChild(d)
      })
      el.appendChild(g)
    })
  ...
  // ── weekly ────────────────────────────────────────────────────────────────
  function renderWeek() {
    const wk = wkK(), el = document.getElementById('w-list')
    if (!el) return
    el.innerHTML = wk.length ? wk.map(k => {
      const s=st(k.k)
      const badge = s==='mastered'?'<span class="wbadge wb-m">Mastered</span>':s==='learning'?'<span class="wbadge wb-l">Learning</span>':'<span class="wbadge wb-s">Seen</span>'
      return '<div class="wrow"><span class="wkj">'+k.k+'</span><div class="winfo"><div class="wmean">'+k.m+'</div><div class="wread">'+k.on+' / '+k.kun+'</div></div>'+badge+'</div>'
    }).join('') : '<p style="color:var(--text3);font-size:.8rem;text-align:center;padding:1rem">No kanji studied this week yet.</p>'
    buildEx()
  }

  function buildEx() {
    const wk=wkK(), pool=wk.length>=4?wk:shuf(allK('all')).slice(0,6)
    const el = document.getElementById('w-ex')
    if (!el) return
    ...
    el.innerHTML = picks.map((k,i) => {
      const others = pool.filter(x=>x.k!==k.k).slice(0,3)
      const opts = shuf([k,...others])
      return `<div class="wqitem"><div class="wqsen" style="font-size:.75rem;color:var(--text2);margin-bottom:.2rem">Fill in: ${k.m} (${k.on})</div><div class="wqsen">「<span class="blank" id="wb${i}">　</span>」— ${k.vocab[0].w} (${k.vocab[0].m})</div><div class="wqopts">${opts.map(o=>`<span class="wqo" onclick="wqA(this,'${o.k}','${k.k}',${i})">${o.k}</span>`).join('')}</div></div>`
    }).join('')
  }

  // expose wqA to window for inline onclick
  if (typeof window !== 'undefined') {
    (window as any).wqA = (btn: HTMLElement, ch: string, cor: string, i: number) => {
      const par = btn.closest('.wqitem') as HTMLElement|null
      if (!par || par.dataset.done) return; par.dataset.done='1'
      par.querySelectorAll('.wqo').forEach((b:Element) => {
        (b as HTMLElement).onclick=null
        if(b.textContent===cor) b.classList.add('ok')
        else if(b===btn && ch!==cor) b.classList.add('bad')
      })
      const bl=document.getElementById('wb'+i); if(bl){bl.textContent=cor;bl.style.color=ch===cor?'var(--n5)':'var(--accent)'}
      toast(ch===cor?'✓ 正解！':'✗ It was: '+cor)
    }
  }
  ...
  return (
    <>
      <div className="topbar">
        <div className="logo"><span>漢字</span> Study</div>
        <div className="pills">
      ...
      <div className="main">
        ...
        <div id="panel-fc" className="panel">
          <div className="fc-meta">
            <span className="fc-ctr" id="fc-ctr">Card 0 of 0</span>
            <button className="btn btn-s btn-sm" onClick={initFC}>↺ Shuffle</button>
          </div>
        ...
        <div id="panel-week" className="panel">
          <div className="card">
            <div className="clabel">THIS WEEK'S KANJI</div>
            <div id="w-list"></div>
          </div>
